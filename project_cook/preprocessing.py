#imports
import pandas as pd
import os

#Mounting from google colab??
# from google.colab import drive
# os.chdir('/content/drive/MyDrive/Colab Notebooks/')
# data = pd.read_csv("Data/recipe_dataset.csv")

filename = 'project_cook/data/full_dataset.csv'
data = pd.read_csv(filename)

#dropping columns
data = data.drop('source', axis = 1)
data = data.drop('Unnamed: 0', axis = 1)

#basic cleaning
def basic_cleaning(text):
  text = text.strip()
  text = text.lower()
  return text

#Removing punctuation
def remove_punctuation(text):
  text = text.translate(str.maketrans('','','"[]'))
  return text

#Simplifying ingredients list
words_to_remove = ['brown', 'breasts','frozen', 'crumbs', 'spread',
                'kosher', 'ground','chopped', 'diced', 'minced',
                'sliced', 'grated', 'shredded','boneless', 'skinless',
                'organic', 'fresh','canned', 'low-fat', 'low-sodium',
                'gluten-free','halal', 'all-purpose','seasoning', 'spice',
                'sauce', 'paste', 'juice', 'zest', 'extract', 'powder',
                'cream', 'sour',  'soy','toasted','roasted', 'smoked',
                'grilled', 'fried', 'baked', 'boiled', 'steamed', 'stir-fried',
                'sauteed', 'white', 'sesame', 'of', 'powdered', 'shredded',
                'size' ]

def remove_words(text):
  for word in words_to_remove:
    text = text.replace(word, '')
  return text.strip()



#applying basic cleaning function
data['NER'] = data['NER'].apply(basic_cleaning)

#Applying the remove punctuation function
data['NER'] = data['NER'].apply(remove_punctuation)
data['ingredients'] = data['ingredients'].apply(remove_punctuation)
data['directions'] = data['directions'].apply(remove_punctuation)

#applying the remove word function
data['NER'] = data['NER'].apply(remove_words)
